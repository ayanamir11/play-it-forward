...
export default function BottomTabBar() {
  ...
  return (
    <nav
      className="lg:hidden fixed bottom-0 left-0 right-0 z-50 flex border-t pb-4"
      ...
      {TABS.map(({ label, href, icon: Icon }) => {
        ...
        return (
          <Link
            key={href}
            href={href}
            className="flex flex-1 flex-col items-center justify-center gap-1 pt-3 text-xs font-medium"
